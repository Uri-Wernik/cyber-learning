...
| **IAM** | *Identity and Access Management*. Controla identidades e permissões dentro da AWS. |
| **EC2** | *Elastic Compute Cloud*. Serviço da AWS usado para criar máquinas virtuais chamadas instâncias. |
| **AMI** | *Amazon Machine Image*. Modelo com sistema e configurações usado para criar uma instância EC2. |
| **EBS** | *Elastic Block Store*. Disco virtual persistente conectado à instância. |
| **Security Group** | Firewall virtual da AWS. É o porteiro que decide qual tráfego pode chegar à instância. |
| **IP e porta** | O IP identifica a máquina na rede; a porta entrega a conexão ao programa correto, como `22`, `80` ou `443`. |
| <span id="dns"></span>**DNS** | *Domain Name System*. Procura um nome e devolve seu IP, como uma agenda de endereços. |
| **A, CNAME e TTL** | `A` liga um nome a um IPv4, `CNAME` cria um apelido e `TTL` define o tempo de cache da resposta. |
| **HTTP** | Protocolo usado para pedidos e respostas na Web. |
| <span id="https"></span>**HTTPS** | É o HTTP protegido por TLS. |
| **TLS** | Cifra a comunicação, detecta alterações e verifica o certificado apresentado pelo servidor. |
| **SSH** | *Secure Shell*. Permite controlar outra máquina por uma conexão criptografada. |
| **2FA e MFA** | Exigem mais de uma prova no login, como senha e código temporário. |
| **Cookie e sessão** | O cookie guarda um identificador; a sessão mantém o usuário reconhecido após o login. |
...
| **C2** | *Command and Control*, em português **Comando e Controle**. Central que envia comandos para agentes remotos e recebe resultados. |
| **Proxy reverso e AiTM** | O proxy fica entre cliente e aplicação. Em um fluxo AiTM, ele mantém uma conexão com cada lado e repassa as mensagens. |
...
| **PHP** | Linguagem executada no servidor para receber dados, aplicar regras e gerar respostas. |
